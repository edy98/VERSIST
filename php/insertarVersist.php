...
function almacenarDatos($ruta){
	...
	foreach ($rss->getElementsByTagName('item') as $node) {
		$itemRSS = array(
			...
			//'issuelinks' => $node->getElementsByTagName('issuelinks')->item(0)->childNodes->item(0)->nodeValue,
			//'attachments' => $node->getElementsByTagName('attachments')->item(0)->nodeValue,
		);
		array_push($feed, $itemRSS);
	}

	//Extrayendo incidencias enlazadas
	...
	foreach ($feed as $itemRSS => $value) {
		...
		/*
		try {
			$sentencia = $connection->prepare("INSERT INTO versist(clave, tipo_incidencia, estatus, componentes, incidencias_enlazadas) VALUES (:key, :type, :status, :component, :issuelinks)");
